...
### 2026-03-26
- `batch_collect.py` 고속화: 수집 시간 약 50% 단축
  - `slow_mo` 300→50ms, 탭 초기 대기 10→3초, 스크롤 대기 1.5→0.5초, `stable_secs` 8→4초
  - 재시도 스크롤 삭제 → API 재수집으로 대체 (불필요한 대기 제거)
  - 1건당 ~5분 → ~2.5분, 10건 기준 ~50분 → ~25분
  - 작업별 소요시간(초) 로그 출력 추가
- `batch_collect.py` 빈응답 검증 추가
  - 수집 후 `result: []`인 차트를 자동 감지하여 재수집 대상에 포함
  - 최대 3라운드 API 재수집 시도, 성공 시 기존 빈 응답 교체
  - 3라운드 모두 빈 응답 시 = 해당 기간에 실제 데이터 없음 (정상)
  - 로그에 빈응답 차트 이름 표시 (디버깅 용이)
- `generate_all_reports()` 분석/비교 기간 자동 분류 개선
  - 기존: 연도 prefix로 구분 (2026=분석, 그 외=비교) → 같은 연도끼리 혼동
  - 변경: 날짜순 정렬 후 최신=분석, 이전=비교로 자동 구분
- 영덕관광 모니터링 리포트 생성 (분석: 03.01~03.25, 비교: 01.01~02.28)
  - 수집: 분석 126/126, 비교 126/126 (100%)
  - 주요 변동: 이동완료 -13.4%, 대기시간 -47.9%(개선), DAU +14.6%, 신규회원 +93.2%
  - 리포트: `shucle_report/영덕관광/20260301_20260325/`
- 양구 동면/해안면 주간 모니터링 리포트 생성 (분석: 03.18~03.25, 비교: 03.11~03.17)
  - 동면: 분석 126/126 KPI 14/14, 비교 126/126 KPI 14/14
  - 해안면: 분석 117/126 KPI 14/14, 비교 126/126 KPI 14/14
  - 동면 경로이탈비중 빈응답 이슈: 첫 수집 시 `result: []` → 재수집으로 해결 (실제 데이터 존재)
  - 리포트: `shucle_report/{동면,해안면}/20260318_20260325/`
- 일평균 계산 모수를 운행일수 기준으로 통일 (총 탑승객 수 vs 대당 탑승객 수 방향 불일치 버그 수정)
  - **문제**: 총 탑승객 차트(`demand_silver`)는 탑승 0명인 날 행이 없어 평균에서 제외, 대당 탑승객 차트(`supply_session_silver`)는 0인 날도 포함 → 두 지표 방향이 반대로 나오는 현상
  - **해결**: `get_operating_days()` 함수 추가 — 운행차량 대수 차트에서 차량>0인 날수를 산출하여 모든 일평균의 분모로 사용
  - `summarize()`, `get_kpi_value()`, `get_drilldown_value()` 모두 `operating_days` 파라미터 추가
  - `monitoring_report.py`: `generate_report()`에서 `op_days`/`prev_op_days` 산출 후 1차/2차/핵심해석 전체에 전달
  - `export_report.py`: `build_report_data()`, `_build_insights()` 동일 적용
  - 리포트 헤더에 운행일수 표시 추가 (예: "운행일: 25일 (비교: 59일)")
  - 영덕관광 검증: 총 탑승객 수 4.1→3.3(감소) → 2.2→2.9(증가, +32.5%)로 수정, 대당 탑승객과 방향 일치
- 오늘 생성된 전체 리포트 재계산 및 재생성 (8지역 11건 HTML/XLSX)
  - 백운면, 봉양읍, 검단신도시, 삼호, 충북혁신도시 (20260101_20260325 vs 20251001_20251230)
  - 영덕관광 (20260301_20260325 vs 20260101_20260228), 영덕관광 (20260101_20260228 단일)
  - 동면, 해안면 (20260311_20260317, 20260318_20260325 양방향)
